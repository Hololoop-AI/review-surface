# Review Surface — UI kit

A clickable design reference for the **Review Surface chrome** that the `review-surface` CLI launches in a browser.
...
Everything is fake — there's no `review-surface` server backing it. The reply text is canned. The DOM snapshot is not transmitted anywhere.
...
- DOM snapshot serialization. `review-surface.snapshot()` would produce something like the tree in `src/artifact-sdk.js`; we don't.
- File-path identity, session store, long-polling.

Read `src/server.js`, `src/chrome-client.js`, `src/chrome.css`, and `src/artifact-sdk.js` in the review-surface repo for the canonical behaviour.
