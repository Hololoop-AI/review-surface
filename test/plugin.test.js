...
function tempDir(prefix = "review-surface-plugin-") {
...
const symlinkSupport = (() => {
  const probe = mkdtempSync(path.join(os.tmpdir(), "review-surface-symlink-probe-"));
  try {
...
test("normalizeRepositoryUrl converts npm git URLs to plain https", () => {
  assert.equal(
    normalizeRepositoryUrl({ url: "git+https://github.com/kunchenguid/review-surface.git" }),
    "https://github.com/kunchenguid/review-surface",
  );
...
test("the package root is itself a discoverable Agent Plugin", async () => {
  ...
  assert.deepEqual(
    ...
    ["review-surface"],
    "exactly the review-surface skill is discovered",
  );

  const skill = await readFile(path.join(root, "skills", "review-surface", "SKILL.md"), "utf8");
  assert.deepEqual(validateSkillMarkdown(skill, { directoryName: "review-surface" }).errors, []);
});

test("the plugin declares no MCP servers", async () => {
  // review-surface's agent surface is the CLI itself; an mcp.json would add a second contract.
...
test("VS Code registration adds the plugin root and is idempotent", () => {
  const [first, changedFirst] = computeVsCodePluginLocationsUpdate({}, "/pkg/review-surface", "review-surface");
  assert.equal(changedFirst, true);
  assert.deepEqual(first["chat.pluginLocations"], { "/pkg/review-surface": true });

  const [second, changedSecond] = computeVsCodePluginLocationsUpdate(first, "/pkg/review-surface", "review-surface");
...
test("VS Code registration preserves unrelated settings and other plugins", () => {
  ...
  const [updated] = computeVsCodePluginLocationsUpdate(settings, "/pkg/review-surface", "review-surface");
  ...
  assert.equal(updated["chat.pluginLocations"]["/pkg/review-surface"], true);
...
test("VS Code registration repairs a relocated install without dropping foreign entries", () => {
  const dir = tempDir();
  const stale = writePlugin(path.join(dir, "old", "review-surface"), "review-surface");
  const foreign = writePlugin(path.join(dir, "other-plugin"), "other-plugin");
  const current = writePlugin(path.join(dir, "new", "review-surface"), "review-surface");

  const settings = { "chat.pluginLocations": { [stale]: true, [foreign]: true } };
  const [updated, changed] = computeVsCodePluginLocationsUpdate(settings, current, "review-surface");

  assert.equal(changed, true);
  assert.equal(updated["chat.pluginLocations"][stale], undefined, "the previous review-surface location is dropped");
...
test("a removed install directory is only treated as stale when it was ours", () => {
  const dir = tempDir();
  assert.equal(isStalePluginLocation(path.join(dir, "gone", "review-surface"), "review-surface"), true);
  assert.equal(isStalePluginLocation(path.join(dir, "gone", "someone-else"), "review-surface"), false);
});
...
test("Cursor registration links, no-ops, and repairs the local plugin slot", { skip: symlinkSupport.skip }, () => {
  ...
  const pluginRoot = writePlugin(path.join(dir, "pkg", "review-surface"), "review-surface");

  const linked = linkCursorLocalPlugin(localPlugins, pluginRoot, "review-surface");
  assert.equal(linked.status, "linked");
  assert.equal(path.resolve(readlinkSync(linked.target)), pluginRoot);

  assert.equal(linkCursorLocalPlugin(localPlugins, pluginRoot, "review-surface").status, "current");

  const moved = writePlugin(path.join(dir, "pkg2", "review-surface"), "review-surface");
  // Exercise Windows' move-aside replacement path even when this suite runs elsewhere.
  const repaired = linkCursorLocalPlugin(localPlugins, moved, "review-surface", { platform: "win32" });
...
test("Cursor registration refuses to clobber a real directory in the slot", () => {
  const dir = tempDir();
  const localPlugins = path.join(dir, "local");
  const occupied = path.join(localPlugins, "review-surface");
  mkdirSync(occupied, { recursive: true });
  writeFileSync(path.join(occupied, "keep.txt"), "user content");

  const result = linkCursorLocalPlugin(localPlugins, path.join(dir, "pkg"), "review-surface");
...
test("Cursor registration replaces a dangling symlink", { skip: symlinkSupport.skip }, () => {
  ...
  symlinkSync(path.join(dir, "vanished"), path.join(localPlugins, "review-surface"));
  const pluginRoot = writePlugin(path.join(dir, "pkg", "review-surface"), "review-surface");

  const result = linkCursorLocalPlugin(localPlugins, pluginRoot, "review-surface");
...
test("Cursor registration preserves the old link when replacement fails", { skip: symlinkSupport.skip }, async () => {
  const dir = tempDir();
  const localPlugins = path.join(dir, "local");
  const original = writePlugin(path.join(dir, "old", "review-surface"), "review-surface");
  const replacement = writePlugin(path.join(dir, "new", "review-surface"), "review-surface");
  mkdirSync(localPlugins, { recursive: true });
  const target = path.join(localPlugins, "review-surface");
  symlinkSync(original, target);

  const result = linkCursorLocalPlugin(localPlugins, replacement, "review-surface", {
    renameSync: () => {
  ...
  assert.deepEqual(await readdir(localPlugins), ["review-surface"]);
});
...
test("Cursor registration reports rather than throws when links cannot be created", () => {
  ...
  const result = linkCursorLocalPlugin(path.join(dir, "local"), path.join(dir, "pkg"), "review-surface", {
    symlinkSync: denied,
  });
...
test("plugin client launch preserves Windows batch arguments", { skip: process.platform !== "win32" }, async () => {
  const dir = tempDir("review-surface plugin client ");
  ...
  const pluginRoot = path.join(dir, "Jane Doe & team", "review-surface");
