...
test("telemetry can be disabled by environment", () => {
  const config = resolveTelemetryConfig({
    env: { REVIEW_SURFACE_TELEMETRY: "0" },
...
test("telemetry sends anonymous Umami event payloads", async () => {
  ...
  assert.match(requests[0].headers["User-Agent"], /^review-surface\/1\.2\.3 telemetry$/);
  assert.deepEqual(requests[0].body.payload.website, "site-1");
  assert.deepEqual(requests[0].body.payload.hostname, "cli");
  assert.deepEqual(requests[0].body.payload.title, "Review Surface CLI");
  assert.deepEqual(requests[0].body.payload.url, "app://review-surface/command");
