...
function chromeSessionData(html) {
  const match = String(html).match(/<script id="review-surface-session" type="application\/json">([\s\S]*?)<\/script>/);
  assert.ok(match);
...
test("server serves chrome browser behavior from a dedicated source file", async () => {
  ...
  assert.match(html, /<script id="review-surface-session" type="application\/json">/);
...
test("the chrome boot failsafe turns the layout gate into a reloadable failure when its script never runs", async () => {
  ...
  const failsafeIndex = boot.html.indexOf("__reviewSurfaceCancelChromeBootFailsafe");
...
test("the chrome client cancels the boot failsafe once it has run", () => {
  ...
  boot.context.window.__reviewSurfaceChromeReady = true;
  boot.context.window.__reviewSurfaceCancelChromeBootFailsafe();
...
test("artifact assets resolve within the artifact directory", async () => {
  const root = path.resolve("/tmp/review-surface-artifact");
...
test("artifact assets reject a symlink that escapes the artifact directory", async () => {
  ...
  const outside = await mkdtemp(path.join(tmpdir(), "review-surface-outside-"));
  try {
...
test("artifact SDK uses a custom annotation card instead of browser prompts", () => {
  ...
  assert.match(js, /review-surface-annotation-card/);
...
test("artifact SDK ignores surface-owned annotation UI", () => {
  const js = createSdkJs("abc");

  assert.match(js, /function isReviewSurfaceUi/);
  assert.match(js, /closest\(["']\[data-review-surface-ui\]["']\)/);
  assert.match(js, /data-review-surface-ui/);
});

test("artifact SDK isolates Review Surface annotation UI in Shadow DOM", () => {
  const js = createSdkJs("abc");

  assert.match(js, /attachShadow\(\{\s*mode:\s*["']open["'],?\s*\}\)/);
  assert.match(js, /:host\{all:initial/);
  assert.match(js, /review-surface-annotation-root/);
});
...
test("annotation mode forces the artifact cursor to default", () => {
  ...
  assert.match(js, /review-surface-cursor-style/);
...
test("artifact SDK registers a capture-phase document keydown listener for the mode toggle hotkey", () => {
  ...
  assert.match(js, /postArtifactMessage\("review-surface:toggleAnnotationMode"\)/);
  ...
  assert.match(
    js,
    /document\.addEventListener\(\s*"keydown",\s*\(event\) => \{\s*if \(!isModeToggleHotkeyEvent\(event\)\) return;\s*event\.preventDefault\(\);\s*postArtifactMessage\("review-surface:toggleAnnotationMode"\);\s*\},\s*true,?\s*\);/,
  );
});
...
test("chrome client toggles annotation mode via Cmd/Ctrl+I and on request from the artifact SDK", async () => {
  ...
  assert.match(js, /if \(msg\.type === "review-surface:toggleAnnotationMode"\) toggleAnnotationMode\(\);/);
  assert.match(
    js,
...
test("annotation card shadow styles use Review Surface design-system variables", () => {
  const js = createSdkJs("abc");
...
test("chrome declares the Review Surface design-system tokens", async () => {
  const css = await chromeCssSource();
...
test("artifact SDK uses design-token aliases for annotation highlight and shadow UI", () => {
  ...
  assert.match(js, /--review-surface-accent:#f4c95d/);
  assert.match(js, /--review-surface-annotate-outline:2px solid var\(--review-surface-accent\)/);
  assert.match(js, /el\.style\.outline\s*=\s*["']var\(--review-surface-annotate-outline,2px solid #f4c95d\)["']/);
  assert.match(js, /el\.style\.outlineOffset\s*=\s*["']var\(--review-surface-annotate-offset,2px\)["']/);
...
test("overflow menu offers publishing an ht-ml.app link via a share dialog", async () => {
  ...
  assert.match(html, /third-party hosting service, not part of Review Surface/);
...
test("artifact SDK reports only stable severe layout failures after fonts, resize, and animations settle", () => {
  ...
  assert.match(js, /postArtifactMessage\(["']review-surface:layoutDiagnostics["']/);
...
test("artifact SDK reports its scroll position and restores it on request", () => {
  ...
  assert.match(js, /postArtifactMessage\(["']review-surface:scroll["']/);
  assert.match(js, /window\.scrollX/);
  assert.match(js, /window\.scrollY/);
  assert.match(js, /msg\.type === ["']review-surface:restoreScroll["']/);
...
test("chrome remembers the artifact scroll position across reloads", async () => {
  ...
  assert.match(js, /msg\.type === ["']review-surface:scroll["']/);
  assert.match(js, /type:\s*["']review-surface:restoreScroll["']/);
  assert.match(js, /x:\s*lastScroll\.x,\s*y:\s*lastScroll\.y/);
});

test("chrome ignores Review Surface postMessages not sent by the artifact iframe", async () => {
  const js = await chromeClientSource();
...
test("chrome restores queued prompts from tab storage after reload", async () => {
  ...
  assert.match(js, /review-surface:queued:/);
...
test("session URLs can disable the layout gate for one open", async () => {
  ...
  try {
    ...
    assert.match(chrome, /<body class="review-surface">/);
...
test("/artifact serves files copied under the artifact directory", async () => {
  const parent = await mkdtemp(path.join(tmpdir(), "review-surface-serve-"));
  const dir = path.join(parent, ".review-surface");
...
test("GET /api/:key/export reports unresolved local asset warning count", async () => {
  ...
  try {
    ...
    assert.equal(exportRes.status, 200);
    assert.equal(exportRes.headers.get("x-review-surface-export-warning-count"), "1");
    assert.equal(exportRes.headers.get("x-review-surface-export-notice-count"), "0");
    assert.equal(exportRes.headers.get("x-review-surface-export-warnings"), null);
...
test("GET /api/:key/export counts notices separately from unresolved assets", async () => {
  ...
  try {
    ...
    assert.equal(exportRes.status, 200);
    assert.equal(exportRes.headers.get("x-review-surface-export-warning-count"), "0");
    assert.equal(exportRes.headers.get("x-review-surface-export-notice-count"), "1");
...
test("POST /api/:key/share is retired: artifacts never leave the machine", async (t) => {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-share-retired-"));
  t.after(() => rm(dir, { recursive: true, force: true }));
  const artifact = path.join(dir, "a.html");
  await writeFile(artifact, "<!doctype html><html><body></body></html>");
  const server = await serve({ port: 0, stateFile: path.join(dir, "state.json"), version: "9.9.9-test" });
  try {
    const base = `http://127.0.0.1:${server.port}`;
    ...
    const { key } = await open.json();
    const share = await fetch(`${base}/api/${key}/share`, {
      method: "POST",
      headers: { "content-type": "application/json", origin: base },
      body: JSON.stringify({}),
    });
    assert.equal(share.status, 410);
    assert.match((await share.json()).error, /remote share disabled/);
  } finally {
    await server.close();
  }
});
...
test("mutating routes reject a present foreign Origin while allowing same-origin and header-less callers", async () => {
  ...
  try {
    ...
    // Destructive/state-revealing GETs get the same guard: a cross-site GET to
    // /api/poll would blind-drain a session's feedback (takeFeedback clears the
    // batch), and /api/:key/export returns full artifact state.
    const foreignPoll = await fetch(`${base}/api/poll?file=${encodeURIComponent(artifact)}&timeoutMs=0`, {
      headers: { origin: "https://attacker.example" },
    });
    assert.equal(foreignPoll.status, 403);
    assert.deepEqual(await foreignPoll.json(), { error: "cross-origin request rejected" });

    const foreignExport = await fetch(`${base}/api/${key}/export`, {
      headers: { referer: "https://attacker.example/page" },
    });
    assert.equal(foreignExport.status, 403);

    // Header-less poll (the CLI long-poll) still works.
    const cliPoll = await fetch(`${base}/api/poll?file=${encodeURIComponent(artifact)}&timeoutMs=0`);
    assert.equal(cliPoll.status, 200);
...
test("POST /api/:key/share rejects cross-origin browser requests", async () => {
  ...
  const previousApiUrl = process.env.REVIEW_SURFACE_HTML_APP_API_URL;
  process.env.REVIEW_SURFACE_HTML_APP_API_URL = `http://127.0.0.1:${htmlApp.port}`;
  ...
  try {
    ...
    await server.close();
    await htmlApp.close();
    restoreEnv("REVIEW_SURFACE_HTML_APP_API_URL", previousApiUrl);
    await rm(dir, { recursive: true, force: true });
  }
});

// Retired endpoint: 410 answers ALL callers before any provenance logic —
// there is nothing to protect on a dead route, and the fake html-app below
// must receive zero requests either way.
test("POST /api/:key/share answers 410 to header-less callers too", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-serve-"));
  const artifact = path.join(dir, "artifact.html");
  await writeFile(artifact, "<!doctype html><title>x</title><h1>Private</h1>\n");
  ...
  try {
    ...
    assert.deepEqual(body, { error: "remote share disabled: artifacts are served locally only" });
    assert.equal(requests.length, 0);
  } finally {
    await server.close();
    await htmlApp.close();
...
test("an agent-initiated end via the file-based route reopens normally without the reopen flag", async () => {
  ...
  try {
    ...
    // `review-surface end <file>` uses the file-based route - agent-initiated.
    await fetch(`${base}/api/end`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ file: artifact }),
...
// #171: a browser tab left open across `review-surface end` must be told the session ended, instead
...
test("hasLiveReloadRootOptIn detects the data attribute and meta opt-in", () => {
  ...
  assert.equal(hasLiveReloadRootOptIn(`<html data-review-surface-live-reload-root><body></body></html>`), true);
  assert.equal(
    hasLiveReloadRootOptIn(`<html><head><meta name="review-surface-live-reload" content="root"></head></html>`),
    true,
  );
});
...
test("resolveWatchTarget defaults to the artifact file so large sibling trees aren't scanned", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-watch-"));
  const artifact = path.join(dir, "artifact.html");
  await writeFile(artifact, "<!doctype html><html><body></body></html>");
  try {
...
test("resolveWatchTarget upgrades to the artifact directory when data-review-surface-live-reload-root opts in", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-watch-"));
  const artifact = path.join(dir, "artifact.html");
  await writeFile(artifact, `<!doctype html><html data-review-surface-live-reload-root><body></body></html>`);
  try {
    const target = await resolveWatchTarget({ file: artifact, key: "abc" });
...
test("resolveWatchTarget falls back to file-only when the artifact can't be read", async () => {
  const target = await resolveWatchTarget({
    file: path.join(tmpdir(), `review-surface-missing-artifact-${process.hrtime.bigint()}.html`),
    key: "abc",
  });
  assert.equal(target.scope, "file");
});
...
test("concurrent same-session opens create only one file watcher", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-watch-race-"));
  const artifact = path.join(dir, "artifact.html");
...
test("/health and / stay responsive after opening two back-to-back sessions", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-back-to-back-"));
...
test("server debug logger receives session and watcher lifecycle events", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-debug-"));
  const artifact = path.join(dir, "artifact.html");
  await writeFile(artifact, "<!doctype html><html><body></body></html>");
...
test("layout gate curtain reuses the ended overlay card styling", async () => {
  ...
  assert.match(noGateHtml, /<body class="review-surface">/);
...
test("annotation card queues and sends immediately on Ctrl+Enter or Cmd+Enter", () => {
  ...
  assert.match(js, /class="review-surface-hint"/);
  assert.match(js, /\+Enter to send/);
  assert.match(js, /\.review-surface-annotation-card \.review-surface-hint\{/);
});
...
test("chrome adopts a favicon tag and tab title passed from the artifact", () => {
  ...
  const html = createChromeHtml(
    { key: "abc", file: "/tmp/artifact.html" },
    { faviconTag, title: "Project Board · Review Surface" },
  );

  assert.ok(html.includes(faviconTag), "artifact favicon tag is injected verbatim");
  assert.match(html, /<title>Project Board · Review Surface<\/title>/);
});
