...
function matchesSelector(el, selector) {
  ...
  if (selector === "[data-review-surface-question]") return el.getAttribute("data-review-surface-question") !== null;
...
test("deriveReviewSurfaceQueueKey uses explicit queueKey first", () => {
  const input = node("input", { type: "radio", name: "plan" });

  assert.equal(deriveReviewSurfaceQueueKey(input, { queueKey: "deployment-plan" }), "deployment-plan");
});

test("deriveReviewSurfaceQueueKey allows explicit empty queueKey to suppress derivation", () => {
  const button = node("button");
  node("section", { "data-review-surface-question": "deployment-plan" }, [button]);

  assert.equal(deriveReviewSurfaceQueueKey(button, { queueKey: "" }), "");
});

test("deriveReviewSurfaceQueueKey groups controls inside data-review-surface-question", () => {
  const first = node("button");
  const second = node("button");
  node("section", { "data-review-surface-question": "deployment-plan" }, [first, second]);

  assert.equal(deriveReviewSurfaceQueueKey(first), "question:deployment-plan");
  assert.equal(deriveReviewSurfaceQueueKey(second), "question:deployment-plan");
});

test("deriveReviewSurfaceQueueKey groups radio options by scoped group name", () => {
  ...
  assert.equal(deriveReviewSurfaceQueueKey(planA), "radio:form:deploy:plan");
  assert.equal(deriveReviewSurfaceQueueKey(planB), "radio:form:deploy:plan");
});

test("deriveReviewSurfaceQueueKey keeps same radio names independent across scopes", () => {
  ...
  assert.notEqual(deriveReviewSurfaceQueueKey(first), deriveReviewSurfaceQueueKey(second));
});

test("deriveReviewSurfaceQueueKey does not infer plain button grouping without question metadata", () => {
  const button = node("button");

  assert.equal(deriveReviewSurfaceQueueKey(button), "");
});

test("deriveReviewSurfaceQueueKey keys checkbox toggles per checkbox, not per group", () => {
...
test("deriveReviewSurfaceQueueKey does not collide checkbox default values", () => {
...
test("deriveReviewSurfaceQueueKey keys named selects as fields", () => {
  const select = node("select", { name: "region" });
  node("form", { id: "deploy" }, [select]);

  assert.equal(deriveReviewSurfaceQueueKey(select), "field:form:deploy:region");
});
