...
async function withSession(run, { env } = {}) {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-attach-srv-"));
...
test("upload rejects bytes over the configured per-image cap with 413", async () => {
  await withSession(
    ...
    { env: { REVIEW_SURFACE_MAX_ATTACHMENT_BYTES: "8" } },
  );
});
...
test("export still works and leaks no attachment data when a prompt references an image", async () => {
  await withSession(async ({ base, key }) => {
    ...
    assert.doesNotMatch(html, /review-surface:uploadAttachment/);
  });
});

test("the server sweeps an expired, unreferenced attachment at startup", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-attach-sweep-"));
  ...
  const saved = process.env.REVIEW_SURFACE_ATTACHMENT_TTL_MS;
  process.env.REVIEW_SURFACE_ATTACHMENT_TTL_MS = "1000";
  ...
  try {
    ...
    await server.close();
    if (saved === undefined) delete process.env.REVIEW_SURFACE_ATTACHMENT_TTL_MS;
    else process.env.REVIEW_SURFACE_ATTACHMENT_TTL_MS = saved;
    await rm(dir, { recursive: true, force: true });
  }
});
