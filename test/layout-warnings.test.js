...
test("the configured diagnostic viewport set falls back to every class", () => {
  ...
  assert.deepEqual(resolveDiagnosticViewportClasses({ REVIEW_SURFACE_DIAGNOSTIC_VIEWPORTS: "desktop, mobile" }), [
    "desktop",
    "mobile",
  ]);
  assert.deepEqual(resolveDiagnosticViewportClasses({ REVIEW_SURFACE_DIAGNOSTIC_VIEWPORTS: "nonsense" }), [
    "mobile",
    "compact",
    "desktop",
  ]);
