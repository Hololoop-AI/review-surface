...
test("the SDK bundle uploads captured images through the chrome", () => {
  ...
  assert.match(sdk, /postArtifactMessage\("review-surface:uploadAttachment", \{/);
  assert.doesNotMatch(sdk, /parent\.postMessage\(\s*\{\s*type: "review-surface:uploadAttachment"/);
...
test("the SDK bundle applies upload results and offers a retry", () => {
  assert.match(sdk, /review-surface:attachmentResult/);
...
test("the SDK bundle renders chips with a thumbnail, name, and status", () => {
  assert.match(sdk, /review-surface-attachment-thumb/);
  assert.match(sdk, /review-surface-attachment-name/);
...
test("the SDK bundle renders a visible, titled remove control on each chip", () => {
  ...
  assert.match(sdk, /review-surface-attachment-remove/);
});
...
test("the count-cap notice reads as an error, not as the passive keyboard hint", () => {
  ...
  assert.match(sdk, /review-surface-hint-alert/);
  assert.match(sdk, /\.review-surface-hint-alert\{[^}]*color:#ff9d7a/);
  assert.match(sdk, /attachNotice\.classList\.add\("review-surface-hint-alert"\)/);
  // Clearing the notice restores the neutral hint instead of leaving stale red text.
  assert.match(sdk, /attachNotice\.classList\.remove\("review-surface-hint-alert"\)/);
});
