...
test(
  ...
  async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "review-surface-layout-browser-"));
    ...
    const chromeEnv = {
      CHROME_DEVTOOLS_AXI_SESSION: `review-surface-layout-${process.pid}`,
      CHROME_DEVTOOLS_AXI_USER_DATA_DIR: path.join(temp, "chrome"),
    };

    function openArtifact(file) {
      const output = run(process.execPath, ["bin/review-surface.js", file, "--no-open"], reviewSurfaceEnv);
