...
// audit returned straight out of `review-surface poll` with no user action, so several individually
...
const runBrowserE2e = process.env.REVIEW_SURFACE_BROWSER_E2E === "1";
...
test(
  ...
  async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "review-surface-warning-inbox-"));
    ...
    const chromeEnv = {
      CHROME_DEVTOOLS_AXI_SESSION: `review-surface-warning-inbox-${process.pid}`,
      CHROME_DEVTOOLS_AXI_USER_DATA_DIR: path.join(temp, "chrome"),
    };

    function openArtifact(file, args = []) {
      const output = run(process.execPath, ["bin/review-surface.js", file, "--no-open", ...args], reviewSurfaceEnv);
    ...
    function poll(file, timeoutMs) {
      return run(
        process.execPath,
        ["bin/review-surface.js", "poll", file, "--timeout-ms", String(timeoutMs)],
        reviewSurfaceEnv,
        timeoutMs + 45_000,
      );
    }
    ...
    function inbox() {
      return evaluate(
        "JSON.stringify({" +
          ...
          " loads: window.__reviewSurfaceArtifactLoads," +
    ...
    function instrumentArtifactLoads() {
      run(
        ...
        [
          "eval",
          '() => { window.__reviewSurfaceArtifactLoads = 0; if (window.__reviewSurfaceArtifactLoadsWired) return "reset"; window.__reviewSurfaceArtifactLoadsWired = true; document.getElementById("artifact").addEventListener("load", () => { window.__reviewSurfaceArtifactLoads += 1; }); return "wired"; }',
        ],
        chromeEnv,
      );
    }
