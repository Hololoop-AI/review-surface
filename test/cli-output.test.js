...
function setupHooksEnv(homeDir, stateDir) {
  ...
  return { ...env, HOME: homeDir, REVIEW_SURFACE_STATE_DIR: stateDir };
}
...
test("home output teaches agents when and how to use Review Surface", () => {
  const output = createHomeOutput({ bin: `${os.homedir()}/.local/bin/review-surface`, sessions: [] });

  assert.equal(output.bin, "~/.local/bin/review-surface");
  assert.match(output.description, /Review Surface/);
  assert.match(output.description, /complex response/);
  assert.match(output.description, /consider using Review Surface/);
  ...
  assert.ok(output.help.some((item) => item.includes("review-surface <html-file>")));
  assert.ok(output.help.some((item) => item.includes("`.review-surface/`")));
  assert.ok(output.help.some((item) => item.includes("review-surface playbook <playbook_id>")));
  ...
  assert.ok(!output.help.some((item) => item.includes('<meta name="review-surface-design" content="off">')));
...
test("the design-priority rule is single-sourced and keeps its three-step semantics", () => {
  ...
  assert.match(DESIGN_SYSTEM_HINT, /review-surface design/);
...
test("open output flags an artifact that never paints its own page surface", () => {
  ...
  assert.match(warned.next_step, /review-surface poll \/tmp\/artifact\.html/, "the poll contract stays intact");
...
test("export and share outputs flag an unpainted page surface before it reaches a host", () => {
  ...
  assert.match(exported.next_step, /no Review Surface server/, "the export contract stays intact");
...
test("home output warns agents that poll needs an observable wake path", () => {
  const output = createHomeOutput({ bin: "review-surface", sessions: [] });
  const pollHelp = output.help.find((item) => item.includes("review-surface poll <html-file>"));
...
test("ambient and per-artifact output never nags about installing the plugin", () => {
  ...
  const home = createHomeOutput({ bin: "review-surface", sessions: [] });
...
test("home output tailors poll guidance when invoked under Codex", () => {
  const output = createHomeOutput({ bin: "review-surface", sessions: [], agent: "codex" });
  const pollHelp = output.help.find((item) => item.includes("review-surface poll <html-file>"));
...
test("home output keeps static skill poll guidance safe and agent-neutral", () => {
  const output = createHomeOutput({ bin: "review-surface", sessions: [], agent: "static" });
  const pollHelp = output.help.find((item) => item.includes("review-surface poll <html-file>"));
...
test("top-level help renders static home output without dynamic sessions", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-help-test-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)), "--help"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
        env: { ...process.env, REVIEW_SURFACE_STATE_DIR: stateDir },
      },
    );

    assert.equal(result.status, 0, result.stderr || result.stdout);
    assert.match(result.stdout, /playbooks\[7\]/);
    assert.match(result.stdout, /review-surface playbook <playbook_id>/);
    ...
    assert.match(result.stdout, /review-surface design/);
    ...
    assert.doesNotMatch(result.stdout, /review-surface-design/);
...
test("playbook detail output returns focused Review Surface-native guidance", () => {
  ...
  assert.ok(output.playbook.design_rules.some((item) => item.includes("data-review-surface-action")));
  assert.ok(output.playbook.design_rules.some((item) => item.includes("data-review-surface-question")));
  assert.ok(output.playbook.design_rules.some((item) => item.includes("queueKey")));
  assert.ok(output.playbook.review_surface_notes.some((item) => item.includes("window.reviewSurface.queuePrompt")));
  assert.ok(output.playbook.review_surface_notes.some((item) => item.includes("onsubmit")));
  ...
  assert.ok(output.playbook.review_surface_notes.some((item) => item.includes("Review Surface")));
});
...
test("unknown playbook ids produce an actionable validation error", () => {
  assert.throws(
    ...
    (error) => {
      ...
      assert.ok(error.suggestions.some((item) => item.includes("review-surface playbook")));
      return true;
    },
  );
});
...
test("home directory collapse tolerates Windows mixed separators", () => {
  assert.equal(
    collapseHomeDirectory("C:\\Users\\runneradmin/.local/bin/review-surface", "C:\\Users\\runneradmin"),
    "~/.local/bin/review-surface",
  );
  assert.equal(
    collapseHomeDirectory("C:\\Users\\runneradmin\\.local\\bin\\review-surface", "C:\\Users\\runneradmin"),
    "~/.local/bin/review-surface",
...
test("open output keeps the user URL in session data and next_step focused on polling", () => {
  ...
  assert.match(output.next_step, /review-surface poll \/tmp\/artifact\.html/);
...
test("a user-ended open refuses with a status agents can branch on, not a URL to open", () => {
  ...
  assert.match(output.next_step, /user explicitly ended this Review Surface session from the browser/);
  ...
  assert.match(output.next_step, /review-surface \/tmp\/artifact\.html --reopen/);
});
...
test("export output reports the written file and reassures it needs no server", () => {
  ...
  assert.match(output.next_step, /no Review Surface server/);
...
test("export command writes a portable HTML file next to the artifact", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/review-surface-export-test-`);
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)), "export", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, REVIEW_SURFACE_STATE_DIR: dir, REVIEW_SURFACE_TELEMETRY: "0" },
        encoding: "utf8",
      },
    );
...
test("export command treats --out value as an option operand, not the source file", async () => {
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)), "export", "--out", output, artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, REVIEW_SURFACE_STATE_DIR: dir, REVIEW_SURFACE_TELEMETRY: "0" },
        encoding: "utf8",
...
test("share help states the retirement and points at export", () => {
  const help = getCommandHelp("share");
  assert.match(help, /Disabled in this build/);
  assert.match(help, /export <html-file>/);
  assert.doesNotMatch(help, /ht-ml\.app/);
});
...
test("poll feedback and the next step are emitted before the bulky DOM snapshot", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-poll-output-test-`);
  ...
  const server = createServer((req, res) => {
    if (req.url === "/health") {
      ...
      res.end(JSON.stringify({ ok: true, app: "review-surface", version: VERSION }));
      return;
    }
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)), "poll", artifact, "--timeout-ms", "1000"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, REVIEW_SURFACE_STATE_DIR: stateDir, REVIEW_SURFACE_PORT: String(address.port) },
      },
    );
    let stdout = "";
    let stderr = "";
...
test("a poll reporting the session ended by the user tells the agent to stop and not reopen", () => {
  ...
  assert.match(output.next_step, /user ended this Review Surface session/);
  assert.match(output.next_step, /Stop polling/);
  assert.match(output.next_step, /do not run `review-surface \/tmp\/report\.html` to reopen it/);
  assert.match(output.next_step, /deliver any remaining updates directly in this conversation/i);
  assert.match(output.next_step, /review-surface \/tmp\/report\.html --reopen/);
});
...
test("a poll reporting an agent-ended session allows a plain reopen if still needed", () => {
  ...
  assert.match(output.next_step, /Stop polling/);
  assert.match(output.next_step, /review-surface \/tmp\/report\.html`\s+to open a fresh session/);
...
test("the final feedback batch before an agent end preserves ended_by and allows plain reopen", () => {
  ...
  assert.equal(output.session.session_ended, true);
  assert.equal(output.session.ended_by, "agent");
  assert.match(output.next_step, /last feedback before the Review Surface session ended/);
  assert.match(output.next_step, /review-surface \/tmp\/report\.html`\s+to open a fresh session/);
  assert.doesNotMatch(output.next_step, /--reopen/);
  assert.doesNotMatch(output.next_step, /user ended this Review Surface session/);
});
...
test("final user-ended feedback still reports a fatal artifact failure without reopening", () => {
  ...
  assert.match(output.next_step, /confirm it renders without reopening this ended Review Surface session/);
  assert.doesNotMatch(output.next_step, /--reopen/);
});
...
test("poll wait messages tell watching agents the silence is normal", () => {
  ...
  assert.match(banner, /\[review-surface\]/);
  ...
  assert.match(tick, /\[review-surface\]/);
  ...
  assert.match(interrupted, /\[review-surface\]/);
  assert.match(interrupted, /Poll interrupted/);
  assert.match(interrupted, /user may still be reviewing/);
  assert.match(interrupted, /review-surface poll \/tmp\/report\.html/);
...
test("spawned poll with piped stderr banners once and leaves re-run guidance when killed", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-poll-wait-test-`);
  const artifact = `${stateDir}/artifact.html`;
  await writeFile(artifact, "<html><body>hello</body></html>", "utf8");
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)), "poll", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, REVIEW_SURFACE_STATE_DIR: stateDir, REVIEW_SURFACE_PORT: String(server.port) },
      },
    );
...
test("waiting next step reassures agents that re-running poll loses nothing", () => {
  ...
  assert.match(output.next_step, /review-surface poll \/tmp\/report\.html/);
  assert.match(output.next_step, /without --timeout-ms/);
  assert.match(output.next_step, /feedback remains queued until delivery/);
});
...
test("setup hooks resolves HOME before platform-specific user profile variables", () => {
  assert.equal(
    resolveHookHomeDir({ HOME: "/tmp/review-surface-home", USERPROFILE: "C:\\Users\\runneradmin" }, "/fallback"),
    "/tmp/review-surface-home",
  );
});
...
test("setup hooks creates a Copilot CLI hook that injects additional context", () => {
  ...
  assert.match(updated.hooks.sessionStart[1].bash, /review-surface/);
...
test("Copilot CLI ambient context script wraps review-surface output as hook JSON", async () => {
  const tempDir = await mkdtemp(`${os.tmpdir()}/review-surface-copilot-hook-`);
  try {
    const fakeCli = path.join(tempDir, "fake-review-surface.js");
    ...
    assert.match(output.additionalContext, /## AXI ambient context: review-surface/);
...
test("setup hooks installs agent session hooks explicitly", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-setup-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-setup-home-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)), "setup", "hooks"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
    ...
    assert.ok(existsSync(`${homeDir}/.copilot/hooks/review-surface.json`));

    const copilotHook = JSON.parse(await readFile(`${homeDir}/.copilot/hooks/review-surface.json`, "utf8"));
...
test("setup hooks exits with an error when hook installation fails", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-setup-fail-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-setup-fail-home-`);
  try {
...
function runSetupPlugin(homeDir, stateDir, pathDir) {
  return spawnSync(
    process.execPath,
    [fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)), "setup", "plugin"],
    {
      cwd: fileURLToPath(new URL("..", import.meta.url)),
      encoding: "utf8",
...
if (command === "plugins list") {
  if (options.invalidList) {
    ...
    const records = [{ kind: "plugin", name: "review-surface-tools", source: "direct" }];
    if (options.installedSource && fs.existsSync(options.installedSource)) {
      records.push(options.listSourcePath
        ? { kind: "plugin", name: "review-surface", sourcePath: fs.readFileSync(options.installedSource, "utf8") }
        : { kind: "plugin", name: "review-surface", source: "direct" });
    }
...
if (command === "plugin install") {
  ...
  if (options.copilotConfig) {
    fs.writeFileSync(options.copilotConfig, JSON.stringify({
      installedPlugins: [{ name: "review-surface", source: { source: "local", path: pluginRoot } }],
    }));
  }
...
test("setup plugin registers the installed package in the clients that are present", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-path-`);
  try {
    ...
    assert.equal(result.status, 0, result.stderr || result.stdout);
    assert.match(result.stdout, /name: review-surface/);
    ...
    const linked = await realpath(`${homeDir}/.cursor/plugins/local/review-surface`);
    assert.equal(linked, await realpath(fileURLToPath(new URL("..", import.meta.url))));
    assert.ok(existsSync(`${linked}/plugin.json`));
    assert.ok(existsSync(`${linked}/skills/review-surface/SKILL.md`));
  } finally {
    await rm(stateDir, { force: true, recursive: true });
    await rm(homeDir, { force: true, recursive: true });
...
test("setup plugin registers VS Code without disturbing existing settings", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-vs-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-vs-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-vs-path-`);
...
test("setup plugin creates VS Code settings for a fresh installation", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-vs-fresh-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-vs-fresh-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-vs-fresh-path-`);
  const settingsFile = resolveVsCodeSettingsFile({}, homeDir);
  try {
    await mkdir(path.dirname(settingsFile), { recursive: true });
...
test("setup plugin leaves unparseable VS Code settings alone", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-jsonc-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-jsonc-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-jsonc-path-`);
  const settingsFile = resolveVsCodeSettingsFile({}, homeDir);
...
test("setup plugin repairs Copilot registration without trusting list text", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-path-`);
  ...
  try {
    ...
    await writeFile(installedSource, "/stale/review-surface");
    await writeFile(
      copilotConfig,
      '{"installedPlugins":[{"name":"review-surface","source":{"source":"local","path":"/stale/review-surface"}}]}',
    );
...
test("setup plugin preserves Copilot registration when replacement fails", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-failure-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-failure-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-failure-path-`);
  const installedSource = path.join(homeDir, "copilot-installed-source");
  const originalSource = "/working/review-surface";
  try {
...
test("setup plugin does not install when Copilot records are invalid", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-invalid-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-invalid-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-copilot-invalid-path-`);
  const installLog = path.join(homeDir, "copilot-install-log");
  try {
...
test("setup plugin isolates a client it cannot register from the ones it can", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-iso-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-iso-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/review-surface-plugin-iso-path-`);
  const settingsFile = resolveVsCodeSettingsFile({}, homeDir);
  try {
    ...
    const occupied = `${homeDir}/.cursor/plugins/local/review-surface`;
...
test("setup rejects an unknown action and names both supported ones", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/review-surface-setup-unknown-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/review-surface-setup-unknown-home-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)), "setup", "everything"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
        env: setupHooksEnv(homeDir, stateDir),
...
test("server entry resolves to a node-executable script that actually invokes run()", () => {
  // Running from source, the entry must be `bin/review-surface.js` (the only file in the
  ...
  // From source: bin/review-surface.js is present and preferred.
  assert.equal(entry, fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)));
});
...
test("shouldRestartServer restarts same-version Review Surface servers when forced", () => {
  assert.equal(shouldRestartServer("0.1.4", { ok: true, app: "review-surface", version: "0.1.4" }, true), true);
...
test("serverReplacementReason names a local-build force apart from a real version change", () => {
  assert.equal(serverReplacementReason("0.1.4", { ok: true, app: "review-surface", version: "0.1.3" }), "upgrade");
  assert.equal(serverReplacementReason("0.1.4", { ok: true, app: "review-surface" }), "upgrade");
  assert.equal(
    serverReplacementReason("0.1.4", { ok: true, app: "review-surface", version: "0.1.4" }, true),
    "local-build",
  );
...
test("serverReplacementReason names nothing when no replacement is warranted", () => {
  assert.equal(serverReplacementReason("0.1.4", { ok: true, app: "review-surface", version: "0.1.4" }), "");
...
test("shouldKillProcessOnPort kills pre-handshake Review Surface servers after shutdown fails", () => {
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true }), true);
});

test("shouldKillProcessOnPort only kills Review Surface servers with a mismatched version", () => {
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true, app: "review-surface", version: "0.1.3" }), true);
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true, app: "review-surface", version: "0.1.4" }), false);
});

test("shutdownServerOnPort kills pre-handshake Review Surface servers when shutdown does not free the port", async () => {
  let shutdowns = 0;
  let kills = 0;
  ...
  const output = await shutdownServerOnPort(4387, {
    ...
    processMatchesReviewSurface: () => true,
  });
...
test("shutdownServerOnPort ignores unidentified health responders", async () => {
  ...
  const output = await shutdownServerOnPort(4387, {
    ...
    processMatchesReviewSurface: () => false,
  });

  assert.equal(shutdowns, 0);
  assert.equal(kills, 0);
  assert.deepEqual(output, { server: { status: "not-review-surface", port: 4387 } });
});
...
test("open can resume a session without opening another browser window", () => {
  ...
  assert.equal(shouldOpenBrowser(["artifact.html"], { REVIEW_SURFACE_NO_OPEN: "1" }), false);
  ...
  assert.match(getCommandHelp("design"), /review-surface design/);
...
test("polling a file without an active session tells the agent to open it first", () => {
  assert.throws(
    ...
    (error) => {
      ...
      assert.match(error.message, /No active Review Surface session/);
      assert.ok(error.suggestions.some((item) => item.includes("review-surface /tmp/report.html")));
      return true;
    },
  );
});

test("network fetch failures become structured Review Surface server errors", async () => {
  await assert.rejects(
    () => fetchJson("http://127.0.0.1:1/api/poll"),
    (error) => {
      assert.ok(error instanceof AxiError);
      assert.equal(error.code, "SERVER_ERROR");
      assert.match(error.message, /Review Surface server connection failed/);
      assert.ok(error.suggestions.some((item) => item.includes("review-surface server --verbose")));
      return true;
    },
  );
});
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) => {
        assert.ok(error instanceof AxiError);
        assert.equal(error.code, "SERVER_ERROR");
        assert.match(error.message, /Review Surface poll response was interrupted/);
        return true;
      },
    );
...
test("stop command shuts down the running server on the configured port", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/review-surface-stop-test-`);
...
async function startShutdownRecorder(version = "0.0.0-previous") {
  ...
  const server = createServer((req, res) => {
    if (req.url === "/health") {
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ ok: true, app: "review-surface", version }));
      return;
    }
...
test("review-surface stop tells the server it was stopped, and names no session to reload", async () => {
...
test("opening an artifact names that session as the one to reload across a version upgrade", async () => {
  const recorder = await startShutdownRecorder();
  const dir = await mkdtemp(path.join(os.tmpdir(), "review-surface-open-reload-"));
  ...
  try {
    ...
    const child = spawn(
      ...
      [
        fileURLToPath(new URL("../bin/review-surface.js", import.meta.url)),
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          REVIEW_SURFACE_PORT: String(recorder.port),
          REVIEW_SURFACE_STATE_DIR: stateDir,
