...
async function startWhiteboardServer() {
  const dir = await mkdtemp(path.join(tmpdir(), "review-surface-wb-server-"));
...
test("createWhiteboardFrameHtml loads only whiteboard-assets resources", () => {
  ...
  assert.match(html, /__reviewSurfaceWhiteboardChannelToken="channel-token"/);
...
test("whiteboard confirms sanitized links inside the frame", async () => {
  ...
  assert.match(css, /data-review-surface-whiteboard-theme="dark"/);
});
...
async function frameChannelToken(base, query = "") {
  ...
  return /__reviewSurfaceWhiteboardChannelToken="([^"]+)"/.exec(frame)?.[1] || "";
}
