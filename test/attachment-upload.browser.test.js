...
// sent review-surface:uploadAttachment via a raw parent.postMessage with no
...
(function () {
  var PNG_B64 = "${PNG_B64}";
  function shadow() {
    var host = document.querySelector(".review-surface-annotation-root");
  ...
  async function drive() {
    try {
      ...
      for (var i = 0; i < 100 && !card; i += 1) {
        await sleep(100);
        var root = shadow();
        card = root ? root.querySelector(".review-surface-annotation-card") : null;
      }
      ...
      for (var k = 0; k < 150; k += 1) {
        await sleep(100);
        var chip = card.querySelector(".review-surface-attachment-chip");
        var error = chip ? chip.querySelector(".review-surface-attachment-status-error") : null;
        ...
        if (chip && !chip.querySelector(".review-surface-attachment-status")) {
          mark("ready");
          textarea.value = "e2e: attachment round trip";
          card.querySelector(".review-surface-send").click();
          mark("queued");
          return;
        }
      }
...
test(
  ...
  async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "review-surface-attach-e2e-"));
    ...
    const chromeEnv = {
      CHROME_DEVTOOLS_AXI_SESSION: `review-surface-attach-e2e-${process.pid}`,
    ...
    try {
      ...
      const poll = run(
        process.execPath,
        ["bin/review-surface.js", "poll", artifact, "--timeout-ms", "20000"],
        reviewSurfaceEnv,
        65_000,
      );
