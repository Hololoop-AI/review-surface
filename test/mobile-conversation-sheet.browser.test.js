...
test(
  ...
  async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "review-surface-mobile-sheet-"));
    ...
    const chromeEnv = {
      CHROME_DEVTOOLS_AXI_SESSION: `review-surface-mobile-sheet-${process.pid}`,
    ...
    try {
      ...
      for (const reply of REPLIES) {
        run(
          process.execPath,
          ["bin/review-surface.js", "poll", artifact, "--agent-reply", reply, "--timeout-ms", "200"],
          reviewSurfaceEnv,
          30_000,
        );
      }
