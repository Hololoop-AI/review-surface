...
test("createSkillMarkdown emits valid frontmatter naming the review-surface skill", () => {
  ...
  assert.equal(frontmatter.name, "review-surface");
...
test("createSkillMarkdown conforms to the Agent Skills frontmatter contract", () => {
  ...
  const { valid, errors } = validateSkillMarkdown(createSkillMarkdown(), { directoryName: "review-surface" });
...
test("createSkillMarkdown handles explicit /review-surface invocation arguments", () => {
...
test("createSkillMarkdown uses non-interactive npx commands", () => {
  ...
  assert.match(md, /`npx -y review-surface <html-file>`/);
  assert.match(md, /If review-surface output shows a follow-up command starting with `review-surface`/);
  assert.match(md, /run it as `npx -y review-surface/);
  assert.doesNotMatch(md, /`npx review-surface/);
  assert.doesNotMatch(md, /Run `review-surface/);
});
...
test("createSkillMarkdown documents installed-copy fallback for restricted sandboxes", () => {
  ...
  assert.match(md, /`node "\$\(npm root\)\/review-surface\/dist\/cli\.mjs" <html-file>`/);
  assert.match(md, /`node "\$\(npm root -g\)\/review-surface\/dist\/cli\.mjs" <html-file>`/);
  assert.match(md, /bare `review-surface <html-file>` bin/);
});
