...
test("installable skill stays in sync with the no-args home output", async () => {
  ...
  const committed = await readFile(new URL("../skills/review-surface/SKILL.md", import.meta.url), "utf8");
...
test("published package includes the installable skill", async () => {
  ...
  assert.ok(packageJson.files.includes("skills/review-surface"));
});
...
test("review-surface-design agent skill is marked internal for skills CLI discovery", async () => {
  const skillMd = await readFile(new URL("../.agents/skills/review-surface-design/SKILL.md", import.meta.url), "utf8");
  const frontmatter = skillMd.slice(4, skillMd.indexOf("\n---\n", 4));

  assert.match(frontmatter, /^name: review-surface-design$/m);
  assert.match(frontmatter, /^metadata:\n {2}internal: true$/m);
});

test("public review-surface skill is not marked internal", async () => {
  const skillMd = await readFile(new URL("../skills/review-surface/SKILL.md", import.meta.url), "utf8");
  const frontmatter = skillMd.slice(4, skillMd.indexOf("\n---\n", 4));
...
test("package metadata matches the GitHub repository used for npm provenance", async () => {
  ...
  assert.equal(packageJson.repository.url, "git+https://github.com/Hololoop-AI/review-surface.git");
  assert.equal(packageJson.bugs.url, "https://github.com/Hololoop-AI/review-surface/issues");
  assert.equal(packageJson.homepage, "https://github.com/Hololoop-AI/review-surface#readme");
});
...
test("release workflow keeps telemetry env during npm publish prepack", async () => {
  ...
  assert.match(
    workflow,
    /run: npm publish --access public --provenance\n\s+if: \$\{\{ steps\.release\.outputs\.release_created \}\}\n\s+env:\n\s+REVIEW_SURFACE_UMAMI_HOST: https:\/\/a\.kunchenguid\.com\n\s+REVIEW_SURFACE_UMAMI_WEBSITE_ID: \$\{\{ vars\.REVIEW_SURFACE_UMAMI_WEBSITE_ID \}\}/,
  );
});
