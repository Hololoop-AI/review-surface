...
test("strips the injected Review Surface SDK script so exports do not phone home to the server", async () => {
...
test("default reader allows trusted root-absolute mapped design assets outside the artifact root", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "review-surface-export-"));
  try {
