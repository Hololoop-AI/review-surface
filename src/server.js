...
export async function serve({
  ...
  // (REVIEW_SURFACE_HOST) or naming a link host (REVIEW_SURFACE_LINK_HOST) adds that host,
  ...
  // REVIEW_SURFACE_ALLOWED_HOSTS; a lone "*" there disables the guard for operators
  ...
  app.post("/api/sessions", async (req, res, next) => {
    try {
      ...
      // `review-surface <file>` is the exact behavior this route exists to prevent - require an
      // explicit `reopen` opt-in instead of reviving it automatically. Agent-initiated ends
      // (`review-surface end`) keep reviving on the next open, same as before this change.
  ...
  // chromes - it never emits "feedback", so it can never make `review-surface poll` return and can
  ...
  // hosting service not part of Review Surface, returning the share URL. Publishing sends the artifact
  ...
  app.post("/api/:key/share", async (req, res, next) => {
    // Remote share is retired, not broken: artifacts never leave the machine
    // via a third-party host (security decision, 2026-08-23). 410 with a plain
    // reason instead of the 502 an unhandled publish throw would produce.
    ...
    try {
      ...
      res.type("html").send(
        createChromeHtml(session, {
          ...
          faviconTag,
          title: title ? `${title} · Review Surface` : "Review Surface",
  ...
  app.get("/events/:key", async (req, res, next) => {
    ...
    try {
      ...
      // A session end (`review-surface end` or the browser's own End/Send & End) must reach every
...
// the resolved bind and link host and any explicit REVIEW_SURFACE_ALLOWED_HOSTS
...
// A lone "*" in REVIEW_SURFACE_ALLOWED_HOSTS is an explicit opt-out of the Host
...
// REVIEW_SURFACE_ALLOWED_HOSTS. This is an AND check: a client-spoofed forwarded host
...
// `data-review-surface-live-reload-root` attribute on its root element or
// `<meta name="review-surface-live-reload" content="root">`.
...
// Pull a tab favicon + title out of the artifact's own <head>. Review Surface renders the
// artifact in a sandboxed iframe, so the artifact's own <link rel="icon"> and
// <title> never reach the browser tab; surfacing them here makes a wall of Review Surface
// tabs identifiable. Falls back to the Review Surface default favicon. Only data: and
