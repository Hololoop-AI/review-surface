...
const sessionDataElement = document.getElementById("review-surface-session");
const sessionData = JSON.parse(sessionDataElement?.textContent || "{}");
const key = String(sessionData.key || "");
const filePath = String(sessionData.file || "");
const queueStorageKey = "review-surface:queued:" + key;
...
const warningSelectionStorageKey = "review-surface:warning-selection:" + key;
...
const reviewStateStorageKey = "review-surface:review-state:" + key;
// Drafts Review Surface could not replay. The text outlives the draft that carried it, so the user can
// still read and copy it after the anchor it was written against is gone for good.
const retiredDraftStorageKey = "review-surface:retired-drafts:" + key;
/** @type {any[]} */
const retiredDraftNodes = [];
const internalQueueKeyField = "_reviewSurfaceQueueKey";
...
// In-iframe review context (an open annotation card's unsent text, surface-owned question
...
// where the server is being replaced (a version-driven restart, or another `review-surface <file>`
...
const HEALTH_NO_ANSWER_TITLE = "Review Surface did not answer.";
const HEALTH_NO_ANSWER_COPY =
  "Review Surface did not answer the check, so this page cannot tell whether it is running. Try again in a moment.";
...
// (REVIEW_SURFACE_MAX_ATTACHMENTS_PER_PROMPT), so a prompt can legitimately carry more
...
// the shutdown names its reason and each one gets its own line - a page told "Review Surface was updated"
...
function chromeOutdatedCopy(reason) {
  if (reason === "upgrade") return "Review Surface was updated. This page is running the previous version.";
  if (reason === "local-build") {
    return "Review Surface was restarted to pick up a local build. This page is running the copy the previous server sent.";
  }
  if (reason === "stop") return "Review Surface was stopped. Reload after you start it again.";
  return "The Review Surface server this page was connected to is no longer running. Reloading will work once it is running again.";
}
...
// Retiring a draft ends Review Surface's ability to replay it, so the text itself is handed back to the
...
function renderRetiredDraft(text, stored = true) {
  ...
  el.innerHTML =
    "<small>Unsent annotation</small><div>The element this note was attached to is no longer in the artifact, so Review Surface could not reopen the card. Your text is kept here:</div>" +
...
function requestSnapshot(action) {
  ...
  postToFrame({ type: "review-surface:requestSnapshot" });
}
...
function setLayoutGateCard(state) {
  ...
  layoutGateCopy.textContent = "Review Surface is waiting for fonts and final geometry before revealing this artifact.";
}
...
function revealWarning(warning) {
  postToFrame({ type: "review-surface:revealElement", selector: warning.selector });
}
...
function markSessionEnded() {
  ...
  revealLayoutGate();
  postToFrame({ type: "review-surface:setAnnotationMode", enabled: false });
...
async function exportArtifact() {
  ...
  try {
    ...
    const warningCount = Number(response.headers.get("x-review-surface-export-warning-count") || "0");
    const noticeCount = Number(response.headers.get("x-review-surface-export-notice-count") || "0");
...
async function replaceArtifactFrame({ recoveryRetry = false } = {}) {
  ...
  if (!artifactSrc) {
    ...
    frame.src = currentSrc + (currentSrc.includes("?") ? "&" : "?") + "reviewSurface_reload=" + Date.now();
    return true;
  }
  const requestSequence = ++artifactLoadRequestSequence;
  const requestId = `review-surface-load-${Date.now().toString(36)}-${requestSequence}-${Math.random().toString(36).slice(2)}`;
  ...
  const recoverLater = () => {
    ...
    if (!artifactLoadToken) {
      setLayoutGateFailure(
        "Review Surface could not load this artifact.",
        "The Review Surface server did not answer this review's load request. It usually restarted while this page was opening. Check and reload to reconnect.",
        "Check and reload",
        checkServerThenReload(
          "Review Surface could not load this artifact.",
          "Review Surface is still not answering. Start it again with your agent, then use Check and reload.",
        ),
      );
    }
    return false;
        ...
        if (status === "superseded") {
          ...
          if (!artifactLoadToken) {
            setLayoutGateFailure(
              ...
              "Review Surface loads an artifact in one tab at a time. Take over here to move the review into this tab, or switch back to the tab that already has it.",
              "Take over here",
            );
          }
...
async function handleWhiteboardReady(index, mode, isCurrent) {
  try {
    ...
    postToWhiteboard(index, mode, {
      type: "review-surface-whiteboard:init",
      mode,
      diagramIndex: index,
...
function showWhiteboardOverlay(index) {
  ...
  whiteboardOverlay.hidden = false;
  postToFrame({ type: "review-surface:suspendWhiteboard", diagramIndex: index });
...
function finishWhiteboardClose(index) {
  ...
  overlayChannelId = "";
  inlineWhiteboardChannels.delete(index);
  if (!ended) postToFrame({ type: "review-surface:resumeWhiteboard", diagramIndex: index });
}
...
function beginWhiteboardTeardown(index, placement, onComplete) {
  ...
  const message = { type: "review-surface-whiteboard:prepareTeardown", flushId };
...
function beginWhiteboardFlush(index, placement) {
  ...
  postToWhiteboard(index, placement, { type: "review-surface-whiteboard:flush", flushId });
  return promise;
}
...
function handleWhiteboardSave(index, message, mode) {
  ...
  saveWhiteboardScene(index, message).then(
    () => {
      if (flushId) postToWhiteboard(index, mode, { type: "review-surface-whiteboard:saveResult", flushId, ok: true });
    },
    (error) => {
      if (flushId) {
        postToWhiteboard(index, mode, {
          type: "review-surface-whiteboard:saveResult",
          flushId,
          ok: false,
...
async function queueWhiteboardFeedback(index, message, mode) {
  ...
  try {
    ...
    pulseSheetDock();
    postToWhiteboard(index, mode, { type: "review-surface-whiteboard:queueResult", ok: true });
    if (mode === "overlay") closeWhiteboard();
  } catch (error) {
    postToWhiteboard(index, mode, {
      type: "review-surface-whiteboard:queueResult",
      ok: false,
      error: error instanceof Error ? error.message : String(error),
    });
  }
  ...
  try {
    ...
    if (nextHash !== record.sourceHash) {
      ...
      postToWhiteboardOverlay({
        type: "review-surface-whiteboard:sourceChanged",
        source: record.source,
        sourceHash: record.sourceHash,
      });
    }
...
function handleAuthenticatedWhiteboardMessage(index, message, mode) {
  if (message.type === "review-surface-whiteboard:save") handleWhiteboardSave(index, message, mode);
  if (message.type === "review-surface-whiteboard:queueFeedback") queueWhiteboardFeedback(index, message, mode);
  if (message.type === "review-surface-whiteboard:maximize" && mode === "inline") openWhiteboardOverlay(index);
  if (message.type === "review-surface-whiteboard:close" && mode === "overlay") closeWhiteboard();
  if (message.type === "review-surface-whiteboard:teardownReady") finishWhiteboardTeardown(index, message, mode);
  if (message.type === "review-surface-whiteboard:teardownFailed") failWhiteboardTeardown(index, message, mode);
  if (message.type === "review-surface-whiteboard:flushComplete") finishWhiteboardFlush(index, message, mode);
}
...
function handleInlineWhiteboardMessage(event, message) {
  ...
  if (index === null) return;
  if (message.type === "review-surface-whiteboard:ready") {
...
// with whatever else the machine is doing, and several `review-surface` invocations can be racing for
// the same port. Reloading on a fixed short deadline regardless of whether anything is listening
// trades a recoverable page for the browser's connection-error page, which no Review Surface code can
...
async function reloadChromeAfterServerRestart(reason = "") {
  ...
  if (!healthy) {
    ...
    setLayoutGateFailure(
      "Review Surface is not running.",
      "The Review Surface server restarted and did not come back. Start it again with your agent, then check and reload this page.",
      "Check and reload",
      checkServerThenReload(
        "Review Surface is not running.",
        "Review Surface is still not running. Start it again with your agent, then use Check and reload.",
      ),
      { sticky: true },
    );
    return;
      ...
      if (outdatedText && generation === chromeOutdatedGeneration) {
        outdatedText.textContent =
          outcome === "no-answer"
            ...
            : "Review Surface is still not running. Start it again, then use Check and reload.";
      }
    }
  }
}
...
async function uploadAttachment(message, reportResult = postToFrame, signal) {
  ...
  if (!Number.isFinite(size) || size < 0) {
    reportResult({
      type: "review-surface:attachmentResult",
      nonce,
      localId,
      ok: false,
...
function toggleAnnotationMode() {
  ...
  postToFrame({ type: "review-surface:setAnnotationMode", enabled: annotation });
}
...
frame.addEventListener("load", () => {
  ...
  postToFrame({ type: "review-surface:setAnnotationMode", enabled: annotation && !ended });
  // Replay the pre-reload scroll position so hot reloads don't jump the artifact to the top.
  postToFrame({ type: "review-surface:restoreScroll", x: lastScroll.x, y: lastScroll.y });
  if (lastReviewState) postToFrame({ type: "review-surface:restoreReviewState", state: lastReviewState });
  if (overlayIndex !== null) {
    inlineWhiteboardChannels.delete(overlayIndex);
    postToFrame({ type: "review-surface:suspendWhiteboard", diagramIndex: overlayIndex });
  }
});
...
chromeBootWindow.__reviewSurfaceChromeReady = true;
chromeBootWindow.__reviewSurfaceCancelChromeBootFailsafe?.();
