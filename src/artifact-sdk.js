...
export const LAVISH_INTERNAL_QUEUE_KEY = "_reviewSurfaceQueueKey";
...
export function deriveReviewSurfaceQueueKey(element, options = {}) {
...
/**
 * Decide whether an incoming `review-surface:attachmentResult` may be applied to this
...
export function createArtifactSdk(
  ...
  // The count cap mirrors the server's REVIEW_SURFACE_MAX_ATTACHMENTS_PER_PROMPT, passed
  ...
  function attachmentChipHtml(item, index) {
    ...
    const thumb = item.url
      ? '<img class="review-surface-attachment-thumb" src="' + escapeAnnotationText(item.url) + '" alt="">'
      : '<span class="review-surface-attachment-thumb review-surface-attachment-thumb-empty" aria-hidden="true"></span>';
    let status = "";
    if (item.status === "uploading") status = '<span class="review-surface-attachment-status">Uploading…</span>';
    else if (item.status === "error")
      status =
        '<span class="review-surface-attachment-status review-surface-attachment-status-error">' +
    ...
    return (
      ...
      '">' +
      thumb +
      '<span class="review-surface-attachment-body"><span class="review-surface-attachment-name" title="' +
      name +
      '">' +
      name +
      "</span>" +
      ...
      "</span>" +
      retry +
      '<button type="button" class="review-surface-attachment-remove" data-attachment-remove="' +
      index +
  ...
  function queuePrompt(prompt, options = {}) {
    ...
    /** @type {{ uid: string, prompt: string, selector: string, tag: string, text: string, target?: unknown, attachments?: Array<{ id: string, name?: string }>, _reviewSurfaceQueueKey?: string }} */
    const item = {
  ...
  function activeDocumentAnimations() {
    ...
    return document
      ...
      .filter((animation) => !isReviewSurfaceUi(animationTarget(animation)));
  }
  ...
  // user typed or answered inside it is lost unless the chrome replays it. Review Surface only ever
  // replays state it owns: an open annotation card, and controls inside a `data-review-surface-question`
  // scope (the documented Review Surface input contract). Application-owned form state is left alone
  // because Review Surface cannot tell an intentional reset from a preserved answer.
  // ---------------------------------------------------------------------------
  ...
  function showAnnotationCard(target, options = {}) {
    ...
    card.innerHTML =
      ...
      placeholder +
      '"></textarea><div class="review-surface-attachments" data-attachments hidden></div>' +
      '<div class="review-surface-attach-row"><button class="review-surface-attach" type="button">' +
      ...
      '<input class="review-surface-attach-input" type="file" accept="' +
      ATTACHMENT_IMAGE_TYPES.accept +
      '" multiple hidden></div>' +
      '<div class="review-surface-hint">Enter to queue &middot; ' +
      sendNowHint +
      "+Enter to send &middot; paste or drop an image" +
      '</div><div class="review-surface-row"><button class="review-surface-cancel" type="button">Cancel</button><button class="review-surface-send" type="button">Queue</button></div>';
    ...
    const cancelButton = /** @type {HTMLButtonElement | null} */ (card.querySelector(".review-surface-cancel"));
    const sendButton = /** @type {HTMLButtonElement | null} */ (card.querySelector(".review-surface-send"));
    const attachmentsList = /** @type {HTMLDivElement | null} */ (card.querySelector("[data-attachments]"));
    const attachButton = /** @type {HTMLButtonElement | null} */ (card.querySelector(".review-surface-attach"));
    const attachInput = /** @type {HTMLInputElement | null} */ (card.querySelector(".review-surface-attach-input"));
    const attachNotice = /** @type {HTMLDivElement | null} */ (card.querySelector(".review-surface-hint"));
    ...
    // Unsent annotation text is review context Review Surface owns, so it is reported to the chrome and
  ...
  /** @type {Window & { reviewSurface?: unknown }} */ (window).reviewSurface = {
    queuePrompt,
    sendQueuedPrompts,
    endSession,
    getQueuedPrompts: () => [],
  ...
  // Bring a warning's element into view and flash it. The marker is Review Surface UI, so it is excluded
  ...
  document.addEventListener(
    ...
    (event) => {
      if (
        ...
        isReviewSurfaceAction(event.target) ||
