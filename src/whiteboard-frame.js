  ...
  chromeWindow.postMessage(
    {
      ...
      ...(state.channelId
        ...
        : {
            channelToken: String(/** @type {any} */ (window).__reviewSurfaceWhiteboardChannelToken || ""),
...
function buildShell(theme, mode) {
  document.body.dataset.reviewSurfaceWhiteboardTheme = theme;
  document.body.dataset.reviewSurfaceWhiteboardMode = mode;
...
function prepareTeardown(message) {
  ...
  if (!postSave(flushId)) {
    ...
    post({ type: "review-surface-whiteboard:teardownReady", flushId });
  }
}
...
function flushSaveNow(message) {
  ...
  if (!postSave(flushId)) {
    ...
    post({ type: "review-surface-whiteboard:flushComplete", flushId, ok: true });
  }
}
...
function handleSaveResult(message) {
  ...
  if (flushId === state.teardownFlushId) {
    ...
    post({ type: "review-surface-whiteboard:teardownFailed", flushId, error });
    return;
  }
  if (state.flushIds.delete(flushId)) {
    post({ type: "review-surface-whiteboard:flushComplete", flushId, ok: Boolean(message.ok) });
  }
}
...
function main() {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (msg.type === "review-surface-whiteboard:sourceChanged") handleSourceChanged(msg);
    if (msg.type === "review-surface-whiteboard:prepareTeardown") prepareTeardown(msg);
    if (msg.type === "review-surface-whiteboard:flush") flushSaveNow(msg);
    if (msg.type === "review-surface-whiteboard:saveResult") handleSaveResult(msg);
    if (msg.type === "review-surface-whiteboard:queueResult") {
      resetQueueButton();
      if (msg.ok) {
  ...
  post({ type: "review-surface-whiteboard:ready" });
}
