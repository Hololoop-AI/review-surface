...
// the `review-surface design` summary, and the design command help. Edit the rule here only;
...
export const DESIGN_SYSTEM_HINT =
  "Review Surface does not auto-inject any design system - artifacts stay portable so they render identically when opened directly without review-surface running. Before writing any HTML: " +
  DESIGN_PRIORITY_RULE +
  " Run `review-surface design` for a content-to-playbook router, a copy-pasteable CDN snippet, a Mermaid CDN snippet/init for diagrams, and the DaisyUI component reference. When you deliver the artifact, state which of the three design sources you used and why.";
...
export function createDesignOutput() {
  ...
  return {
    ...
    design: {
      ...
      layout_safety_note:
        "Optional copy-paste CSS for artifacts with dense nested grid/flex layouts, badges, wide monospace or pixel fonts, or local media. Paste it into the artifact yourself when useful. Review Surface never auto-injects it, so direct-open portability stays intact.",
      other_design_systems:
        "If the user asks for a different design system (Bootstrap, custom CSS, plain HTML, etc.), use that instead - Review Surface does not require DaisyUI.",
    },
    diagram_tooling: {
      use_when:
