...
const DEFAULT_TITLE = "Review Surface CLI";
...
export function resolveTelemetryConfig(input) {
  ...
  const host =
    String(input.env.REVIEW_SURFACE_UMAMI_HOST || "").trim() || input.buildHost.trim() || HARDCODED_FALLBACK_HOST;
  return { enabled: true, host, websiteID };
}

export function getBuildTimeUmamiHost() {
  return process.env.REVIEW_SURFACE_BUILD_UMAMI_HOST || "";
}

export function getBuildTimeUmamiWebsiteID() {
  return process.env.REVIEW_SURFACE_BUILD_UMAMI_WEBSITE_ID || "";
}
