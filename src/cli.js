...
const DESCRIPTION =
  "Review Surface helps agents turn rich HTML artifacts into collaborative human review surfaces. Whenever you are about to give user a complex response that will be easier to understand via a rich / interactive page, consider using Review Surface. " +
  "First generate an interactive HTML artifact according to user request, then run `review-surface <html-file>` so the user can visually review it, annotate elements or selected text, queue prompts, and send feedback back through `review-surface poll`.";
...
export const VERSION =
  process.env.REVIEW_SURFACE_BUILD_VERSION ||
...
export function createOpenOutput({ file, url, status, agent = "generic", selfPaintWarning = undefined }) {
  const selfPaintPrefix = selfPaintWarning
    ? `First fix the unpainted page surface flagged in self_paint_warning and save - Review Surface live-reloads the artifact automatically, so you do not need to re-run \`review-surface ${file}\`. `
    : "";
  return {
...
// Shown when a plain `review-surface <file>` targets a session the user explicitly ended from the
...
export function pollWaitBannerText(file) {
  return (
    `[review-surface] Long-polling for user feedback on ${file}. This stays silent until the user sends feedback or ends the session - leave it running. ` +
    ...
    `If it gets killed or times out before feedback arrives, re-run \`review-surface poll ${file}\` - feedback remains queued until delivery. Poll delivery consumes the response, so read it completely.`
  );
}
...
export function pollInterruptedText(file) {
  return (
    `[review-surface] Poll interrupted before user feedback arrived. The user may still be reviewing - ` +
    `re-run \`review-surface poll ${file}\` to keep waiting; feedback remains queued until delivery. Poll delivery consumes the response, so read it completely.`
  );
}
...
function createFeedbackNextStep(file, artifactFailures, sessionEnded, endedBy, prompts = [], agent = "generic") {
  ...
  const whiteboardNote = prompts.some((prompt) => prompt && prompt.tag === "whiteboard")
    ? `This feedback includes whiteboard edits (tag "whiteboard"): read the edit summary in the prompt text first, and only when it is not enough, open the target's scenePath (.excalidraw scene JSON) or previewPath (PNG) local files for detail. The artifact's Mermaid source stays authoritative - apply the edits by updating the Mermaid text in ${file} (Review Surface live-reloads it); never try to write the .excalidraw scene back. `
    : "";
  ...
  if (sessionEnded) {
    const failureNote =
      count > 0
        ? endedBy === "user"
          ? `${count} fatal artifact failure${count === 1 ? "" : "s"} arrived alongside this final feedback - the review surface itself could not be used. Repair ${file}, then open it directly and confirm it renders without reopening this ended Review Surface session. `
          : `${count} fatal artifact failure${count === 1 ? "" : "s"} arrived alongside this final feedback - the review surface itself could not be used. Repair ${file}, then run \`review-surface ${file}\` to open a fresh session. `
        : "";
    if (endedBy === "user") {
      const reopenNote =
        count > 0
          ? ""
          : ` Only run \`review-surface ${file} --reopen\` if the user explicitly asks for further review or something genuinely important needs their visual attention.`;
...
// as-is for the browser to load, so the export needs network to render those. Review Surface makes no
...
// RETIRED: remote publishing is disabled in this build (see SECURITY-NOTES.md).
// The original upstream behavior, kept for reference of what the dead code below did:
// HTML as `export` (remote refs left as links), then POSTs it to ht-ml.app's `/v1/sites` API,
// sending the artifact to ht-ml.app's servers. The service is not part of Review Surface, needs no
...
async function shareCommand(args) {
  // Retired, not broken: see SECURITY-NOTES.md. Export is the sanctioned path.
  return {
    status: "disabled",
...
export function createShareOutput({ source, site, warnings, passwordProtected = false, selfPaintWarning = undefined }) {
  ...
  const hostNote =
    "ht-ml.app (https://ht-ml.app), a third-party host not part of Review Surface, hosts the page, so it needs no Review Surface server.";
  if (unresolved.length) {
    result.next_step =
...
// Explicitly shut down the running Review Surface server. Unlike `end` (which closes a single
...
export function createCopilotCliAmbientContextScript(command = "review-surface") {
  return [
...
export function createCopilotCliSessionStartHook(command = "review-surface", timeoutSec = 10) {
...
export function installCopilotCliSessionStartHook({
  ...
  const target = path.join(hookDir, "review-surface.json");
  try {
...
// macOS/Linux only - Windows users would need to kill manually, but review-surface isn't
...
// run(). In source layout that's `../bin/review-surface.js` (which calls run on import). In the
...
export function resolveServerEntry() {
  const binEntry = fileURLToPath(new URL("../bin/review-surface.js", import.meta.url));
