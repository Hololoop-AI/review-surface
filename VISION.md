...
`review-surface` exists so that a person reviewing what an agent rendered can point at it and be understood exactly.
...
Nothing else is injected, because an artifact must render identically when it is opened directly with Review Surface absent.
...
The artifact matches the project it is about before it matches anything Review Surface would prefer.
Review Surface may offer building blocks that are correct by construction, and they stay opt-in, written in by the artifact's author, and reachable when Review Surface is not running.
An artifact that arrives styled at random is a defect in Review Surface's guidance before it is a defect in the agent.
...
Detection is passive: Review Surface files what it finds where the user can see it, and only the user selecting an issue turns it into work.
...
Work a capable agent already does is not rebuilt inside Review Surface and then explained back to it.

## The instructions are the product

Review Surface is a CLI an agent discovers by running it, so its output is an interface and not documentation.
...
When agents behave badly, changing what Review Surface tells them is a real fix and is preferred over new machinery.
Guidance steers the agent's judgment and stops short of rigid rules it can satisfy without thinking.
Review Surface keeps its own files in its own places and never modifies a file the user or another tool owns without being asked, because that trust is spent once.
...
Review Surface reviews HTML, and it does not adopt Markdown, PDFs, or images by wrapping them in a page that has no saved file behind it.
...
It expects to run inside an agent harness, and the long poll is how that harness holds a continuous session; Review Surface does not launch, drive, or supervise the agent itself.
