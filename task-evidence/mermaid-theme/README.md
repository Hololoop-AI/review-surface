...
Task: make Mermaid diagrams in Review Surface artifacts respect the artifact page theme instead of
rendering in a fixed theme.

All screenshots were captured from the real browser view through the real `review-surface` flow
...
## AFTER (theme-aware init from `review-surface design`)
...
`review-surface <file>` and toggle Dark/Light.
